...
def get_action_from_user(world):
    try:
        s = input('Choose action ([move|shoot|climb|quit], direction): ')
        if s in ['exit', 'quit', 'q']:
            print('Good bye...')
            sys.exit(0)
        a = s.split(' ')
        action = {'move':world.move, 'shoot':world.shoot, 'climb':world.climb}[a[0]]
...
def kb_initialize(kb, world):
    ...
    for r in range(len(world.mapa)):
        for c in range(len(world.mapa[0])):
            ...
            kb.tell(Implication([NOT('wumpus_at', r, c), NOT('pit_at', r, c)], L('safe', r, c)))
            for dr, dc in [(-1, 0), (0, 1), (1, 0), (0, -1)]:
                # There`s no Wumpus if there`s no stench besides
                kb.tell(Implication([NOT('stench', r+dr, c+dc)], NOT('wumpus_at', r, c)))
                # There`s no pit if there`s no breeze besides
                kb.tell(Implication([NOT('breeze', r+dr, c+dc)], NOT('pit_at', r, c)))

def kb_add_step(world, percept):
    # Every time after player makes a step: Add percepts and/or other knowledge to KB
    # Based on the game mechanics, the inference will not detect the square as 'safe' even if the wumpus is killed, due
    # to the stench around.
...
if __name__ == "__main__":
    print_unvisited = True # Only print the tiles I`ve been to. Make True for debugging

    kb = KB()
    world = WumpusWorld('mapa1.txt')
